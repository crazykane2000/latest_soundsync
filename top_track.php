                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="ms_heading">
                                <h1>Top Picked Songs</h1>
                                <span class="hstry_clear ms_btn"><a href="free_music.php">Free Music  </a></span>
                            </div>
                        </div>
                        <?php $pdo = new PDO($dsn, $user, $pass, $opt);   
                            foreach ($data as $key => $value) {
                               // print_r($value);
                                ...
                                try {
                                    $stmt = $pdo->prepare('SELECT * FROM `purchse_track` WHERE `track_id`="'.$value['songId'].'" AND `buyer`="'.$_SESSION['user'].'"  ORDER BY date DESC');
